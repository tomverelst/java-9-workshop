...
public class ProcessAPIImprovements {
    public static void main(String[] args) throws IOException {
        ...
        ProcessBuilder ls = new ProcessBuilder()
                .command("ls")
                .directory(Paths.get("/Users/tomverelst/Desktop").toFile());
        ProcessBuilder grepJpg = new ProcessBuilder()
                .command("grep", "jpg")
                .redirectOutput(ProcessBuilder.Redirect.INHERIT);
        List<Process> processes = ProcessBuilder
                .startPipeline(Arrays.asList(ls, grepJpg));
        ...
        ProcessHandle.allProcesses()
                .filter(process -> process.info().command().filter(command ->
                        command.toLowerCase().contains("slack")).isPresent())
